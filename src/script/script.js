const sellerResource = require('../api/requests/resources');
const fs = require('fs')

    module.exports = async function getResponse() {
        let itemsOfSeller = await sellerResource.getItems(process.argv[2]); //Obtengo todos los items del vendedor atraves del id ingresado en la terminal
        ...
        console.log(arrOfItems) //Devuelvo el array con toda la información requerida.
        ...
        if (arrOfItems.length > 0) {
            fs.writeFileSync(`seller${process.argv[2]}.txt`, JSON.stringify(arrOfItems, null, 2), 'utf-8') //Creo un log del vendedorID con todos los items
        }
